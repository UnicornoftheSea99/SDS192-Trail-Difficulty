...
trail_ratings <- trail_ratings %>%
  mutate(is_difficult = case_when(
    between(rating, 0, 50) ~ "Easy",
    between(rating, 50, 150) ~ "Moderate",
    between(rating, 150, 300) ~ "Difficult",
  ))
...
- `r standard_button("wrangling")`: We met the Wrangling standard since we used tools like group_by(), summarize(), and mutate() in order to manipulate our data for our purposes.
...
- `r standard_button("aesthetics")`: Markdown - We met the Markdown standard since the code we’ve written compiles properly, and we haven’t included things that would slow down the code unnecessarily, like the loading statements.
- `r standard_button("markdown")`: We met the R Markdown standard since the code we’ve written compiles properly, and we haven’t included things that would slow down the code unnecessarily, like the loading statements.
